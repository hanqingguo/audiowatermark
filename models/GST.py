...
class GST(nn.Module):
    ...
    def forward(self, inputs):
        enc_out = self.encoder(inputs)  # reference encoder
...
class STL(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.embed = nn.Parameter(torch.FloatTensor(hp.token_num, hp.E // hp.num_heads))  # [10, E//8]
        d_q = hp.E // 2
    ...
    def forward(self, inputs):
        N = inputs.size(0)
        query = inputs.unsqueeze(1)  # [N, 1, E//2]  reference embedding, "what are we looking for"
        keys = F.tanh(self.embed).unsqueeze(0).expand(N, -1, -1)  # [N, token_num, E // num_heads]  Learnable parameter "what the element offering"
        style_embed = self.attention(query, keys)  # [1, 1, 256]
        # print ("style embed size is : {}".format(style_embed.shape))
